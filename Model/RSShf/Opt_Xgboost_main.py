...
# Grid search for XGBoost hyperparameters using 5-fold cross-validation
param_grid = {
...
# Create the final XGBoost model using the best parameters from grid search
best_params = grid_search.best_params_
xgb_model = XGBRegressor(objective='reg:squarederror', random_state=42, predictor='cpu_predictor', **best_params)
...
Y_pred_cv = cross_val_predict(xgb_model, X_scaled, Y, cv=kf)
...
plt.plot(Y_pred, label='XGB Prediction (Train)', color='red', linestyle='--')
plt.plot(Y_pred_cv, label='XGB Prediction (CV)', color='orange', linestyle='-.')
...
pred_plot_filename = os.path.join(output_dir, "xgb_predictions.png")
plt.savefig(pred_plot_filename, dpi=300)
plt.close()

# Save the trained model and the scaler for future predictions
model_filename = os.path.join(output_dir, "xgb_model.pkl")
...
spatial_plot_filename = os.path.join(output_dir, "xgb_spatial.png")
