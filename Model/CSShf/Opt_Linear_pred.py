...
model_path = os.path.join(output_dir, "poly_linear_regression_model.pkl")
scaler_path = os.path.join(output_dir, "scaler.pkl")
poly_path = os.path.join(output_dir, "poly_transformer.pkl")
poly_model = joblib.load(model_path)
...
# Transform the features using the saved polynomial transformer
...
print("Loaded Poly Regression Model - Training RMSE:", rmse)
print("Loaded Poly Regression Model - Training R2:", r2)

# Plot predictions vs. actual values
plt.figure(figsize=(10, 6))
plt.plot(Y_LES, label='Actual eddy_double_LES', color='green')
plt.plot(Y, label='Actual eddy_double_TBNN', color='blue')
plt.plot(Y_pred, label='Poly Regression Prediction', color='red', linestyle='--')
plt.xlabel('Sample Index')
plt.ylabel('Value')
plt.title('Poly Regression Prediction vs. Actual')
plt.legend()
plt.grid(True)

# Get the next version number for the plot filename
version = get_next_version(output_dir, "poly_regression_predictions", ".png")
plot_filename = os.path.join(output_dir, f"poly_regression_predictions_{version:03d}.png")
...
spatial_version = get_next_version(output_dir, "poly_regression_spatial", ".png")
spatial_plot_filename = os.path.join(output_dir, f"poly_regression_spatial_{spatial_version:03d}.png")
