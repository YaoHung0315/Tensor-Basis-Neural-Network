...
# Grid search for SVR hyperparameters using 5-fold cross-validation
param_grid = {
    'C': [0.1, 1, 10, 100],
...
# Create the SVR model using the best parameters from grid search
best_params = grid_search.best_params_
svr_model = SVR(kernel='rbf', **best_params)

# Evaluate the model using 5-fold cross-validation
kf = KFold(n_splits=5, shuffle=True, random_state=42)
cv_scores = cross_val_score(svr_model, X_scaled, Y, cv=kf, scoring='neg_mean_squared_error')
cv_rmse = np.sqrt(-np.mean(cv_scores))
print("Cross-validated RMSE:", cv_rmse)
Y_pred_cv = cross_val_predict(svr_model, X_scaled, Y, cv=kf)
print("Cross-validated R2:", r2_score(Y, Y_pred_cv))

# Train the final SVR model on the full dataset
...
plt.plot(Y_pred, label='SVR Prediction (Train)', color='red', linestyle='--')
plt.plot(Y_pred_cv, label='SVR Prediction (CV)', color='orange', linestyle='-.')
...
pred_plot_filename = os.path.join(output_dir, "svr_predictions.png")
...
spatial_plot_filename = os.path.join(output_dir, "svr_spatial.png")
