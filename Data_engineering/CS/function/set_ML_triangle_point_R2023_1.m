...
Hmax_values = linspace(0.1, 0.15, 25);
mesh_point_count = zeros(length(Hmax_values), 1);

for i = 1:length(Hmax_values)
    mesh = generateMesh(model, 'Hmax', Hmax_values(i), 'GeometricOrder', 'quadratic', Hgrad=1.11, Hedge={[2], 0.0975, [3], 0.0585});
...
mesh = generateMesh(model, 'Hmax', optimal_Hmax,  'GeometricOrder', 'quadratic', Hgrad=1.11, Hedge={[2], 0.0975, [3], 0.0585});
% pdeplot(mesh);
