...
Hmax_values = linspace(0.085, 0.14, 25);
mesh_point_count = zeros(length(Hmax_values), 1);

for i = 1:length(Hmax_values)
    mesh = generateMesh(model, 'Hmax', Hmax_values(i), 'GeometricOrder', 'quadratic', Hgrad=1.1, Hedge={[2], 0.0775, [3], 0.07});
...
mesh = generateMesh(model, 'Hmax', optimal_Hmax,  'GeometricOrder', 'quadratic', Hgrad=1.1, Hedge={[2], 0.0775, [3], 0.07});
% pdeplot(mesh);
